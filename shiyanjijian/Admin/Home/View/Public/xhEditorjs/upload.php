...
if($err==''){
	...
	if(preg_match('/^('.str_replace(',','|',$upExt).')$/i',$extension))
	{
		...
		else
		{
			...
			$http_referer = "http://" . $_SERVER['SERVER_NAME'];
			$php_self = dirname($_SERVER['PHP_SELF']);
			$upload_path = $http_referer . $php_self;
			//$upload_path = 'http://localhost/sunlence/1/Application/Admin/View/Public/xhEditorjs';
			$url = $upload_path.'/'.$attachDir.'/'.$newFilename;
			rename($tempPath,$targetPath);
			@chmod($targetPath,0755);
			$url=jsonString($url);
			if($immediate=='1')$targetPath='!'.$targetPath;
			if($msgType==1)$msg="'$url'";
			else $msg="{'url':'".$url."','localname':'".jsonString($localName)."','id':'1'}";//id参数固定不变，仅供演示，实际项目中可以是数据库ID
		}
	}
